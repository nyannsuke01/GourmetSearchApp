  ...
  override func viewDidLoad() {
    ...
    if let lat = shop.lat {
      if let lon = shop.lon {
        ...
        let mkcr = MKCoordinateRegion(center: cllc, latitudinalMeters: 500, longitudinalMeters: 500)
  ...
  override func viewWillAppear(_ animated: Bool) {
    ...
    observers.append(
      nc.addObserver(forName: .didUpdateLocation, object: nil, queue: nil, using: {
        ...
        let mkr = MKCoordinateRegion(center: center, latitudinalMeters: 500, longitudinalMeters: 500)
        self.map.setRegion(mkr, animated: true)
